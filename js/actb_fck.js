function actb(obj, obj_sel, obj_up, filter){
	/* ---- Public Variables ---- */
	...
	function actb_generate(){
		...
		b.style.left = eval(curLeft(actb_curr) + 252) + "px";
	...
	function actb_goup(){
		...
		document.getElementById('tat_div').innerHTML = tAjax.getFileResume(actb_self.actb_ids[actb_pos - 1]);
	...
	function actb_penter(){
		...
		//actb_selected.value = parent.InstanceUrl () + 'titan.php?target=openFile&amp;fileId=' + actb_self.actb_ids[i];
		
		loadFile (actb_self.actb_ids[i], '');
	...
	function actb_tocomplete(kc){
		...
		if (ot.length == actb_self.actb_startcheck)
			eval (tAjax.getSuggest (actb_curr.value, actb_filter));
