...
/**
 * Titan Framework specific functions
 */
function titan_image_extension ($url) {
	$fileId = substr ($url, strrpos ($url, 'fileId=') + 7, strlen ($url));
	
	if (trim ($fileId) == '' || !is_numeric ($fileId))
		return 'jpg';
	...
	try
	{
		...
		$sth = $db->prepare ("SELECT _name FROM _file WHERE _id = '". $fileId ."'");
		...
		return array_pop (explode ('.', $obj->_name));
	}
