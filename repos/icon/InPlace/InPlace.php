...
{	
	...
	public function makeIcon ($itemId, $forceDisable = FALSE)
	{
		...
		return '<img src="'.  $this->iconImage ($this->image) .'" class="icon" border="0" title="'. $this->label .'" alt="'. $this->label .'"  onclick="JavaScript: inPlaceAction (\''. $itemId .'\', \''. $this->action->getName () .'\', \''. $this->section->getName () .'\', this);" />';
	}
